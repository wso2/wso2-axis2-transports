 ...
 */
public class OracleAQMessageSender {

    private static final Log log = LogFactory.getLog(OracleAQMessageSender.class);
     ...
     */
    public OracleAQMessageSender(Connection connection, Session session, MessageProducer producer,
    ...
    /**
     * Create a OracleAQSender using a OracleAQConnectionFactory and target EPR
     *
     * @param jmsConnectionFactory the OracleAQConnectionFactory
     * @param targetAddress target EPR
     */
    public OracleAQMessageSender(OracleAQConnectionFactory jmsConnectionFactory, String targetAddress) {
        ...
        try {
            ...
            this.destination =
                    jmsConnectionFactory.getSharedDestination() == null ?
                            jmsConnectionFactory.getDestination(OracleAQUtils.getDestination(targetAddress),
                                    isQueue ? OracleAQConstants.DESTINATION_TYPE_QUEUE : OracleAQConstants.DESTINATION_TYPE_TOPIC) :
    ...
    public void send(Message message, MessageContext msgCtx) {
        ...
        Boolean persistent   = getBooleanProperty(msgCtx, OracleAQConstants.JMS_DELIVERY_MODE);
        Integer priority     = getIntegerProperty(msgCtx, OracleAQConstants.JMS_PRIORITY);
        Integer timeToLive   = getIntegerProperty(msgCtx, OracleAQConstants.JMS_TIME_TO_LIVE);
        Integer messageDelay = getIntegerProperty(msgCtx, OracleAQConstants.JMS_MESSAGE_DELAY);
        ...
        try {
            ...
            try {
                ...
                if (msgId != null) {
                    msgCtx.setProperty(OracleAQConstants.JMS_MESSAGE_ID, msgId);
                }
     ...
     */
    public void close() {
        if (producer != null && cacheLevel < OracleAQConstants.CACHE_PRODUCER) {
            try {
                producer.close();
            } catch (JMSException e) {
