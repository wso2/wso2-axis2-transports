...
public abstract class OracleAQChannel implements AxisServiceConfigurator {
    private final String name;
    private final String destinationType;
    private final ContentTypeMode contentTypeMode;
    protected @Transient
    OracleAQTestEnvironment env;
    ...
    public OracleAQChannel(String name, String destinationType, ContentTypeMode contentTypeMode) {
        this.name = name;
        this.destinationType = destinationType;
        this.contentTypeMode = contentTypeMode;
    }
    
    public OracleAQChannel(String destinationType, ContentTypeMode contentTypeMode) {
    ...
    protected String buildDestinationName(String direction, String destinationType) {
        ...
        destinationName.append(destinationType == OracleAQConstants.DESTINATION_TYPE_QUEUE ? 'Q' : 'T');
        return destinationName.toString();
    }
    
    protected String buildJndiName(String direction, String destinationType) {
        return "oracleaq/" + buildDestinationName(direction, destinationType);
    }
    
    @Setup @SuppressWarnings("unused")
    private void setUp(OracleAQTestEnvironment env, JNDIEnvironment jndiEnvironment, OracleAQTransportDescriptionFactory tdf) throws Exception {
        this.env = env;
    ...
    public EndpointReference getEndpointReference() throws Exception {
        return new EndpointReference(
                "oracleaq:/" + jndiName + "?transport.oracleaq.DestinationType=" + destinationType +
                "&java.naming.factory.initial=org.mockejb.jndi.MockContextFactory" +
                "&transport.oracleaq.ConnectionFactoryJNDIName=" + connectionFactoryJNDIName +
                "&" + OracleAQConstants.CONTENT_TYPE_PROPERTY_PARAM + "=Content-Type");
    }

    public void setupService(AxisService service, boolean isClientSide) throws Exception {
        service.addParameter(OracleAQConstants.PARAM_JMS_CONFAC, connectionFactoryName);
        service.addParameter(OracleAQConstants.PARAM_DEST_TYPE, destinationType);
        service.addParameter(OracleAQConstants.PARAM_DESTINATION, jndiName);
    }
}
