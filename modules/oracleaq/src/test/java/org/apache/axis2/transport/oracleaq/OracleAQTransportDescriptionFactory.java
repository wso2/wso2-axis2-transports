...
public class OracleAQTransportDescriptionFactory implements TransportDescriptionFactory {
     ...
     */
    public OracleAQTransportDescriptionFactory(boolean singleCF, boolean cfOnSender, int concurrentConsumers) {
    ...
    private void setUp(OracleAQTestEnvironment env, JNDIEnvironment jndiEnvironment) throws Exception {
    ...
    private void setupConnectionFactoryConfig(ParameterInclude trpDesc, String name, String connFactName, String type) throws AxisFault {
        OMElement element = createParameterElement(OracleAQConstants.DEFAULT_CONFAC_NAME, null);
        element.addChild(createParameterElement(Context.INITIAL_CONTEXT_FACTORY,
                MockContextFactory.class.getName()));
        element.addChild(createParameterElement(OracleAQConstants.PARAM_CONFAC_JNDI_NAME,
                connFactName));
        if (type != null) {
            element.addChild(createParameterElement(OracleAQConstants.PARAM_CONFAC_TYPE, type));
        }
        element.addChild(createParameterElement(OracleAQConstants.PARAM_CONCURRENT_CONSUMERS,
    ...
    public TransportInDescription createTransportInDescription() throws Exception {
        TransportInDescription trpInDesc = new TransportInDescription(OracleAQListener.TRANSPORT_NAME);
        setupTransport(trpInDesc);
        trpInDesc.setReceiver(new OracleAQListener());
        return trpInDesc;
    }
    
    public TransportOutDescription createTransportOutDescription() throws Exception {
        TransportOutDescription trpOutDesc = new TransportOutDescription(OracleAQSender.TRANSPORT_NAME);
        if (cfOnSender) {
            setupTransport(trpOutDesc);
        }
        trpOutDesc.setSender(new OracleAQSender());
        return trpOutDesc;
    }
