 ...
 */
public class OracleAQSender extends AbstractTransportSender implements ManagementSupport {

    public static final String TRANSPORT_NAME = Constants.TRANSPORT_JMS;

    private static Map<Transaction, ArrayList<OracleAQMessageSender>> JMSMessageSenderMap = new HashMap<>();
    /** The JMS connection factory manager to be used when sending messages out */
    private OracleAQConnectionFactoryManager connFacManager;
    ...
    public void init(ConfigurationContext cfgCtx, TransportOutDescription transportOut) throws AxisFault {
        ...
        connFacManager = new OracleAQConnectionFactoryManager(transportOut);
     ...
     */
    private OracleAQConnectionFactory getJMSConnectionFactory(OracleAQOutTransportInfo trpInfo) {
        Map<String,String> props = trpInfo.getProperties();
        if (trpInfo.getProperties() != null) {
            String jmsConnectionFactoryName = props.get(OracleAQConstants.PARAM_JMS_CONFAC);
            if (jmsConnectionFactoryName != null) {
                return connFacManager.getJMSConnectionFactory(jmsConnectionFactoryName);
            } else {
                OracleAQConnectionFactory fac = connFacManager.getJMSConnectionFactory(props);
                if (fac == null) {
                    fac = connFacManager.getJMSConnectionFactory(OracleAQConstants.DEFAULT_CONFAC_NAME);
                }
                return  fac;
            }
        } else {
    ...
    public void sendMessage(MessageContext msgCtx, String targetAddress,
        ...
        OracleAQConnectionFactory jmsConnectionFactory = null;
        OracleAQOutTransportInfo jmsOut = null;
        OracleAQMessageSender messageSender = null;

        if (targetAddress != null) {

            jmsOut = new OracleAQOutTransportInfo(targetAddress);
            ...
            if (jmsConnectionFactory != null) {
                messageSender = new OracleAQMessageSender(jmsConnectionFactory, targetAddress);

            } else {
                try {
                    messageSender = jmsOut.createJMSSender(msgCtx);
                    Transaction transaction = (Transaction) msgCtx.getProperty(OracleAQConstants.JMS_XA_TRANSACTION);
                    ...
                    try {
                        if (msgCtx.getProperty(OracleAQConstants.JMS_XA_TRANSACTION_MANAGER) != null) {
                            transaction =
                                    ((TransactionManager) msgCtx.getProperty(OracleAQConstants.JMS_XA_TRANSACTION_MANAGER))
                                            .getTransaction();
                    ...
                    handleException("Unable to create a OracleAQMessageSender for : " + outTransportInfo, e);
                }
            }

        } else if (outTransportInfo != null && outTransportInfo instanceof OracleAQOutTransportInfo) {

            jmsOut = (OracleAQOutTransportInfo) outTransportInfo;
            try {
                messageSender = jmsOut.createJMSSender(msgCtx);
            } catch (JMSException e) {
                handleException("Unable to create a OracleAQMessageSender for : " + outTransportInfo, e);
        ...
        String contentTypeProperty =
            (String) msgCtx.getProperty(OracleAQConstants.CONTENT_TYPE_PROPERTY_PARAM);
        ...
        // into the msgCtx
        String hyphenSupport = OracleAQConstants.DEFAULT_HYPHEN_SUPPORT;
        if (jmsOut.getProperties() != null && jmsOut.getProperties().get(OracleAQConstants.PARAM_JMS_HYPHEN_MODE) != null) {
            if (jmsOut.getProperties().get(OracleAQConstants.PARAM_JMS_HYPHEN_MODE).equals(OracleAQConstants.HYPHEN_MODE_REPLACE)) {
                hyphenSupport = OracleAQConstants.HYPHEN_MODE_REPLACE;
            } else if (jmsOut.getProperties().get(OracleAQConstants.PARAM_JMS_HYPHEN_MODE).equals(OracleAQConstants.HYPHEN_MODE_DELETE)) {
                hyphenSupport = OracleAQConstants.HYPHEN_MODE_DELETE;
            }
        }
        msgCtx.setProperty(OracleAQConstants.PARAM_JMS_HYPHEN_MODE, hyphenSupport);
     ...
     */
    private void sendOverJMS(MessageContext msgCtx, OracleAQMessageSender messageSender,
        String contentTypeProperty, OracleAQConnectionFactory jmsConnectionFactory,
        OracleAQOutTransportInfo jmsOut) throws AxisFault {
        ...
        try {
            ...
            Transaction transaction = null;
            try {
                transaction = ((TransactionManager) msgCtx.getProperty(OracleAQConstants.JMS_XA_TRANSACTION_MANAGER)).getTransaction();
                rollbackXATransaction(transaction);
            } catch (SystemException e1) {
                handleException("Error occurred during obtaining  transaction", e1);
            }
        ...
        if (waitForResponse) {
            ...
            replyDestName = (String) msgCtx.getProperty(OracleAQConstants.JMS_REPLY_TO);
            ...
            String replyDestType = (String) msgCtx.getProperty(OracleAQConstants.JMS_REPLY_TO_TYPE);
            ...
            replyDestination = OracleAQUtils.setReplyDestination(
        ...
        } else if ((replyDestName = (String) msgCtx.getProperty(OracleAQConstants.JMS_REPLY_TO)) != null) {

            String replyDestinationType = (String) msgCtx.getProperty(OracleAQConstants.JMS_REPLY_TO_TYPE);
        ...
        try {
            metrics.incrementBytesSent(msgCtx, OracleAQUtils.getMessageSize(message));
        } catch (JMSException e) {
    ...
    private void waitForResponseAndProcess(Session session, Destination replyDestination,
        ...
        try {
            ...
            consumer = OracleAQUtils.createConsumer(session, replyDestination,
                "JMSCorrelationID = '" + correlationId + "'");

            // how long are we willing to wait for the sync response
            long timeout = OracleAQConstants.DEFAULT_JMS_TIMEOUT;
            String waitReply = (String) msgCtx.getProperty(OracleAQConstants.JMS_WAIT_REPLY);
            if (waitReply != null) {
            ...
            AdtMessage reply = (AdtMessage)consumer.receive(timeout);
            ...
            if (reply != null) {
                ...
                try {
                    metrics.incrementBytesReceived(OracleAQUtils.getMessageSize(reply));
                } catch (JMSException e) {
                    log.warn("Error reading JMS message size to update transport metrics", e);
                }
     ...
     */
    private AdtMessage createJMSMessage(MessageContext msgContext, Session session,
            String contentTypeProperty) throws JMSException, AxisFault {

        AdtMessage message = null;
        String msgType = getProperty(msgContext, OracleAQConstants.JMS_MESSAGE_TYPE);
        ...
        if (jmsPayloadType == null) {
            ...
            boolean useBytesMessage =
                msgType != null && OracleAQConstants.JMS_BYTE_MESSAGE.equals(msgType) ||
            ...
            try {
                ...
                Transaction transaction = null;
                try {
                    transaction = ((TransactionManager) msgContext.getProperty(OracleAQConstants.JMS_XA_TRANSACTION_MANAGER)).getTransaction();
                    rollbackXATransaction(transaction);
                } catch (SystemException e1) {
                    handleException("Error occurred during obtaining  transaction", e1);
                }
        ...
        } else if (OracleAQConstants.JMS_BYTE_MESSAGE.equals(jmsPayloadType)) {
            message = (AdtMessage)session.createBytesMessage();
        ...
        } else if (OracleAQConstants.JMS_TEXT_MESSAGE.equals(jmsPayloadType)) {
            message = (AdtMessage)session.createTextMessage();
            TextMessage txtMsg = (TextMessage) message;
            txtMsg.setText(msgContext.getEnvelope().getBody().
                getFirstChildWithName(BaseConstants.DEFAULT_TEXT_WRAPPER).getText());
        } else if (OracleAQConstants.JMS_MAP_MESSAGE.equalsIgnoreCase(jmsPayloadType)){
            message = (AdtMessage)session.createMapMessage();
            OracleAQUtils.convertXMLtoJMSMap(msgContext.getEnvelope().getBody().getFirstChildWithName(
                    OracleAQConstants.JMS_MAP_QNAME),(MapMessage)message);
        }

        // set the JMS correlation ID if specified
        String correlationId = getProperty(msgContext, OracleAQConstants.JMS_COORELATION_ID);
        ...
        OracleAQUtils.setTransportHeaders(msgContext, message);
        return message;
    }
     ...
     * @return OracleAQConstants.JMS_BYTE_MESSAGE or OracleAQConstants.JMS_TEXT_MESSAGE or null
     */
    private String guessMessageType(MessageContext msgContext) {
        OMElement firstChild = msgContext.getEnvelope().getBody().getFirstElement();
        if (firstChild != null) {
            if (BaseConstants.DEFAULT_BINARY_WRAPPER.equals(firstChild.getQName())) {
                return OracleAQConstants.JMS_BYTE_MESSAGE;
            } else if (BaseConstants.DEFAULT_TEXT_WRAPPER.equals(firstChild.getQName())) {
                return OracleAQConstants.JMS_TEXT_MESSAGE;
            } else if (OracleAQConstants.JMS_MAP_QNAME.equals(firstChild.getQName())){
                return  OracleAQConstants.JMS_MAP_MESSAGE;
            }
        }
        return null;
    }
     ...
     * @throws AxisFault on error
     */
    private void processSyncResponse(MessageContext outMsgCtx, AdtMessage message,
            String contentTypeProperty) throws AxisFault {

        MessageContext responseMsgCtx = createResponseMessageContext(outMsgCtx);

        // load any transport headers from received message
        OracleAQUtils.loadTransportHeaders(message, responseMsgCtx);

        String contentType = contentTypeProperty == null ? null
                : OracleAQUtils.getProperty(message, contentTypeProperty);

        try {
            OracleAQUtils.setSOAPEnvelope(message, responseMsgCtx, contentType);
        } catch (JMSException ex) {
            throw AxisFault.makeFault(ex);
        }

        handleIncomingMessage(responseMsgCtx, OracleAQUtils.getTransportHeaders(message, responseMsgCtx),
                              OracleAQUtils.getProperty(message, BaseConstants.SOAPACTION), contentType);
    }
    ...
    private void commitXATransaction(Transaction transaction) throws AxisFault {
        ArrayList<OracleAQMessageSender> msgSenderList = (ArrayList) JMSMessageSenderMap.get(transaction);
        if (msgSenderList.size() > 0) {
            for (OracleAQMessageSender msgSender : msgSenderList) {
                try {
