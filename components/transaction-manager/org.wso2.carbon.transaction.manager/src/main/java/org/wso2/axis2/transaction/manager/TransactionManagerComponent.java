 ...
 * @scr.reference name="transaction" interface="javax.transaction.Transaction"
 * cardinality="1..1" policy="dynamic" bind="setTransaction"  unbind="unsetTransaction"
...
public class TransactionManagerComponent {
    ...
    protected void setTransaction(Transaction transaction) {
        synchronized (txManagerComponentLock) {
            if (log.isDebugEnabled()) {
                log.debug("Setting the Transaction Service");
            }
            TransactionManagerComponent.transaction = transaction;
        }
    }

    protected void unsetTransaction(Transaction transaction) {
        synchronized (txManagerComponentLock) {
            if (log.isDebugEnabled()) {
                log.debug("Unsetting the Transaction Manager Service");
            }
            TransactionManagerComponent.transaction = null;
        }
    }
    ...
    protected static void bindTransactionManagerWithJNDIForTenant(int tid) {
        ...
        try {
            ...
            Object txManager = null, userTx = null, tx = null;
            try {
            ...
            try {
                tx = currentCtx.lookup("java:comp/Transaction");
            } catch (NameNotFoundException ignore) {
                //ignore
            }
            if(txManager == null) {
            ...
            if(tx == null) {
                currentCtx.bind("Transaction", getTransaction());
            }
        } catch (Exception e) {
