 ...
 *
 * When used for sending messages out, the OracleAQConnectionFactory'ies are able to cache
 * a Connection, Session or Producer
 */
public class OracleAQConnectionFactory {

    private static final Log log = LogFactory.getLog(OracleAQConnectionFactory.class);
    ...
    private AQjmsConnectionFactory conFactory = null;
    ...
    /** The shared JMS connection for this JMS connection factory */
    private int cacheLevel = OracleAQConstants.CACHE_CONNECTION;
     ...
     */
    public OracleAQConnectionFactory(Parameter parameter) {
        ...
        try {
            ...
            conFactory = OracleAQUtils.lookup(context, AQjmsConnectionFactory.class,
                parameters.get(OracleAQConstants.PARAM_CONFAC_JNDI_NAME));
            if (parameters.get(OracleAQConstants.PARAM_DESTINATION) != null) {
                sharedDestination = OracleAQUtils.lookup(context, Destination.class,
                    parameters.get(OracleAQConstants.PARAM_DESTINATION));
            }
            ...
            throw new AxisJMSException("Cannot acquire JNDI context, JMS Connection factory : " +
                parameters.get(OracleAQConstants.PARAM_CONFAC_JNDI_NAME) + " or default destination : " +
                parameters.get(OracleAQConstants.PARAM_DESTINATION) +
                " for JMS CF : " + name + " using : " + OracleAQUtils.maskAxis2ConfigSensitiveParameters(parameters), e);
        }
    ...
    private void digestCacheLevel() {
        ...
        if ("none".equalsIgnoreCase(val)) {
            this.cacheLevel = OracleAQConstants.CACHE_NONE;
        } else if ("connection".equalsIgnoreCase(val)) {
            this.cacheLevel = OracleAQConstants.CACHE_CONNECTION;
        } else if ("session".equals(val)){
            this.cacheLevel = OracleAQConstants.CACHE_SESSION;
        } else if ("producer".equals(val)) {
            this.cacheLevel = OracleAQConstants.CACHE_PRODUCER;
        } else if ("consumer".equals(val)) {
            this.cacheLevel = OracleAQConstants.CACHE_CONSUMER;
        } else if (val != null) {
    ...
    private void setMaxSharedJMSConnectionsCount(){
        if(parameters.get(OracleAQConstants.MAX_JMS_CONNECTIONS_) != null){
            String maxConnectionCount = parameters.get(OracleAQConstants.MAX_JMS_CONNECTIONS_);
            try {
    ...
    public String getReplyToDestination() {
        return parameters.get(OracleAQConstants.PARAM_REPLY_DESTINATION);
    }
    ...
    public String getReplyDestinationType() {
        return parameters.get(OracleAQConstants.PARAM_REPLY_DEST_TYPE) != null ?
                parameters.get(OracleAQConstants.PARAM_REPLY_DEST_TYPE) :
                OracleAQConstants.DESTINATION_TYPE_GENERIC;
    }
    ...
    public boolean isJmsSpec11() {
        return parameters.get(OracleAQConstants.PARAM_JMS_SPEC_VER) == null ||
            OracleAQConstants.JMS_SPEC_VERSION_1_1.equals(parameters.get(OracleAQConstants.PARAM_JMS_SPEC_VER));
    }
    ...
    public boolean isJmsSpec20() {
        return OracleAQConstants.JMS_SPEC_VERSION_2_0.equals(parameters.get(OracleAQConstants.PARAM_JMS_SPEC_VER));
    }
    ...
    public Boolean isQueue() {
        if (parameters.get(OracleAQConstants.PARAM_CONFAC_TYPE) == null &&
            parameters.get(OracleAQConstants.PARAM_DEST_TYPE) == null) {
            return null;
        }

        if (parameters.get(OracleAQConstants.PARAM_CONFAC_TYPE) != null) {
            if ("queue".equalsIgnoreCase(parameters.get(OracleAQConstants.PARAM_CONFAC_TYPE))) {
                return true;
            } else if ("topic".equalsIgnoreCase(parameters.get(OracleAQConstants.PARAM_CONFAC_TYPE))) {
                return false;
            } else {
                throw new AxisJMSException("Invalid " + OracleAQConstants.PARAM_CONFAC_TYPE + " : " +
                    parameters.get(OracleAQConstants.PARAM_CONFAC_TYPE) + " for JMS CF : " + name);
            }
        } else {
            if ("queue".equalsIgnoreCase(parameters.get(OracleAQConstants.PARAM_DEST_TYPE))) {
                return true;
            } else if ("topic".equalsIgnoreCase(parameters.get(OracleAQConstants.PARAM_DEST_TYPE))) {
                return false;
            } else {
                throw new AxisJMSException("Invalid " + OracleAQConstants.PARAM_DEST_TYPE + " : " +
                    parameters.get(OracleAQConstants.PARAM_DEST_TYPE) + " for JMS CF : " + name);
            }
        }
    }
    ...
    private boolean isSessionTransacted() {
        return parameters.get(OracleAQConstants.PARAM_SESSION_TRANSACTED) != null &&
            Boolean.valueOf(parameters.get(OracleAQConstants.PARAM_SESSION_TRANSACTED));
    }

    private boolean isDurable() {
        if (parameters.get(OracleAQConstants.PARAM_SUB_DURABLE) != null) {
            return Boolean.valueOf(parameters.get(OracleAQConstants.PARAM_SUB_DURABLE));
        }
        return false;
    }

    private boolean isSharedSubscription() {
        if (parameters.get(OracleAQConstants.PARAM_IS_SHARED_SUBSCRIPTION) != null) {
            return Boolean.valueOf(parameters.get(OracleAQConstants.PARAM_IS_SHARED_SUBSCRIPTION));
        }
        return false;
    }

    private String getClientId() {
        return parameters.get(OracleAQConstants.PARAM_DURABLE_SUB_CLIENT_ID);
    }
    ...
    private Connection createConnection() {
        ...
        try {
            connection = OracleAQUtils.createConnection(
                conFactory,
                parameters.get(OracleAQConstants.PARAM_JMS_USERNAME),
                parameters.get(OracleAQConstants.PARAM_JMS_PASSWORD),
            ...
            handleException("Error acquiring a Connection from the JMS CF : " + name +
                " using properties : " + OracleAQUtils.maskAxis2ConfigSensitiveParameters(parameters), e);
        }
        return connection;
    }
    ...
    private Session createSession(Connection connection) {
        try {
            ...
            return OracleAQUtils.createSession(
                connection, isSessionTransacted(), Session.AUTO_ACKNOWLEDGE, jmsSpecVersion(), isQueue());

        } catch (JMSException e) {
            try {
                clearSharedConnections();
                return OracleAQUtils.createSession(getConnection(), isSessionTransacted(), Session.AUTO_ACKNOWLEDGE,
    ...
    private MessageProducer createProducer(Session session, Destination destination) {
        try {
            ...
            return OracleAQUtils.createProducer(
    ...
    public Connection getConnection() {
        if (cacheLevel > OracleAQConstants.CACHE_NONE) {
    ...
    public Session getSession(Connection connection) {
        if (cacheLevel > OracleAQConstants.CACHE_CONNECTION) {
    ...
    public MessageProducer getMessageProducer(
        ...
        if (cacheLevel > OracleAQConstants.CACHE_SESSION) {
