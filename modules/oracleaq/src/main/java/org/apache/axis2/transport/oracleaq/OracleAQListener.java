 ...
 * {@see OracleAQConstants.DEFAULT_CONFAC_NAME}
 * <p>
 ...
 */
public class OracleAQListener extends AbstractTransportListenerEx<OracleAQEndpoint> implements ManagementSupport,
    TransportErrorSource {

    public static final String TRANSPORT_NAME = "oracleaq";

    /** The OracleAQConnectionFactoryManager which centralizes the management of defined factories */
    ...
    protected void doInit() throws AxisFault {
        connFacManager = new OracleAQConnectionFactoryManager(getTransportInDescription());
        log.info("JMS Transport Receiver/Listener initialized...");
    }

    @Override
    protected OracleAQEndpoint createEndpoint() {
        return new OracleAQEndpoint(this, workerPool);
    }
     ...
     */
    @Override
    protected void startEndpoint(OracleAQEndpoint endpoint) throws AxisFault {
                ...
                for (int i = 0; i < 3; i++) {
                    ...
                    if (stm.getConsumerCount() > 0) {
                        log.info("Started to listen on destination : " + stm.getDestinationJNDIName() +
                                " of type " + OracleAQUtils.getDestinationTypeAsString(stm.getDestinationType()) +
    ...
    private boolean checkJMSConnection(ServiceTaskManager stm) {
        ...
        try {
            ...
            try {
                jmsConFactory = OracleAQUtils.lookup(
     ...
     */
    @Override
    protected void stopEndpoint(OracleAQEndpoint endpoint) {
        ServiceTaskManager stm = endpoint.getServiceTaskManager();
     ...
     * @return the OracleAQConnectionFactory to be used, or null if reference is invalid
     */
    public OracleAQConnectionFactory getConnectionFactory(AxisService service) {

        Parameter conFacParam = service.getParameter(OracleAQConstants.PARAM_JMS_CONFAC);
        ...
        if (conFacParam != null) {
            ...
            return connFacManager.getJMSConnectionFactory(OracleAQConstants.DEFAULT_CONFAC_NAME);
        }
    }
    ...
    public void pause() throws AxisFault {
        ...
        try {
            for (OracleAQEndpoint endpoint : getEndpoints()) {
