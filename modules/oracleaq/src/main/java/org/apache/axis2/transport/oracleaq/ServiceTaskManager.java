...
public class ServiceTaskManager {
    ...
    // TODO: this overlaps with OracleAQEndpoint#jndiDestinationName; needs to be clarified
    ...
    // TODO: this overlaps with OracleAQEndpoint#destinationType; needs to be clarified
    ...
    private int cacheLevel = OracleAQConstants.CACHE_AUTO;
    ...
    private String jmsSpec = OracleAQConstants.JMS_SPEC_VERSION_1_0;
    ...
    */
    ...
    private OracleAQMessageReceiver oracleAQMessageReceiver = null;
    ...
    public synchronized void start() {
        ...
        for (int i=0; i<concurrentConsumers; i++) {
            if (oracleAQMessageReceiver.getOracleAQListener().getState() == BaseConstants.PAUSED) {
    ...
    private class MessageListenerTask implements Runnable, ExceptionListener {
        ...
        public void pause() {
            if (isActive()) {
                if (connection != null && cacheLevel < OracleAQConstants.CACHE_CONNECTION) {
                    try {
                        connection.stop();
                    } catch (JMSException e) {
        ...
        public void run() {
            ...
            try {
                while (isActive() &&
                    ...
                    AdtMessage message = receiveMessage();
         ...
         */
        private AdtMessage receiveMessage() {
            ...
            try {
                if (getReceiveTimeout() < 0) {
                    AdtMessage msg =  (AdtMessage)consumer.receive();
                    consumerRetryCount = 0;
                    return msg;
                } else {
                    AdtMessage msg = (AdtMessage)consumer.receive(getReceiveTimeout());
                    consumerRetryCount = 0;
                    return msg;
                }
         ...
         */
        private void handleMessage(AdtMessage message, UserTransaction ut) {
            ...
            try {
                commitOrAck = oracleAQMessageReceiver.onMessage(message, ut);
         ...
         */
        private AQjmsSession getSession() {
            if (session == null || cacheLevel < OracleAQConstants.CACHE_SESSION) {
        ...
        private MessageConsumer getMessageConsumer() {
            if (consumer == null || cacheLevel < OracleAQConstants.CACHE_CONSUMER) {
        ...
        private void closeSession(boolean forced) {
            if (session != null &&
                (cacheLevel < OracleAQConstants.CACHE_SESSION || forced)) {
                try {
                    if (log.isDebugEnabled()) {
        ...
        private void closeConsumer(boolean forced) {
            if (consumer != null &&
                (cacheLevel < OracleAQConstants.CACHE_CONSUMER || forced)) {
                try {
                    if (log.isDebugEnabled()) {
        ...
        private Connection createConnection() {
            try {
                conFactory = OracleAQUtils.lookup(
                ...
                handleException("Error looking up connection factory : " + getConnFactoryJNDIName() +
                    " using JNDI properties : " + OracleAQUtils.maskAxis2ConfigSensitiveParameters(jmsProperties), e);
            }
            ...
            try {
                connection = OracleAQUtils.createConnection(
                    conFactory,
                    jmsProperties.get(OracleAQConstants.PARAM_JMS_USERNAME),
                    jmsProperties.get(OracleAQConstants.PARAM_JMS_PASSWORD),
				...
				String msg =
				             "Error acquiring a JMS connection to : " + getConnFactoryJNDIName() +
				                     " using JNDI properties : " + OracleAQUtils.maskAxis2ConfigSensitiveParameters
                                     (jmsProperties) + ". " +
				                     e.getMessage();
				if (log.isDebugEnabled()) {
					log.error(msg, e);
         ...
         */
        private AQjmsSession createSession() {
            try {
                if (log.isDebugEnabled()) {
        ...
        private MessageConsumer createConsumer() {
            ...
            try {
                ...
                log.info("Agent: " + getDurableSubscriberName());

                MessageConsumer consumer = OracleAQUtils.createConsumer(
    ...
    private Destination getDestination(Session session) {
        if (destination == null) {
            try {
                ...
                destination = OracleAQUtils.lookupDestination(context, getDestinationJNDIName(),
                        OracleAQUtils.getDestinationTypeAsString(destinationType));
                if (log.isDebugEnabled()) {
                ...
                try {
                    switch (destinationType) {
                        case OracleAQConstants.QUEUE: {
                            destination = session.createQueue(getDestinationJNDIName());
                            break;
                        }
                        case OracleAQConstants.TOPIC: {
                    ...
                    handleException("Error looking up JMS destination and auto " +
                            ...
                            " using JNDI properties : " + OracleAQUtils.maskAxis2ConfigSensitiveParameters(jmsProperties),
                            e);
                }
            }
        }
    ...
    private UserTransaction getUserTransaction() {
        if (!cacheUserTransaction) {
            ...
            try {
                context = getInitialContext();
                return
                    OracleAQUtils.lookup(context, UserTransaction.class, getUserTransactionJNDIName());
            } catch (NamingException e) {
    ...
    public Boolean isQueue() {
        if (destinationType == OracleAQConstants.GENERIC) {
            return null;
        } else {
            return destinationType == OracleAQConstants.QUEUE;
        }
    }
    ...
    public void setOracleAQMessageReceiver(OracleAQMessageReceiver oracleAQMessageReceiver) {
        this.oracleAQMessageReceiver = oracleAQMessageReceiver;
    }
