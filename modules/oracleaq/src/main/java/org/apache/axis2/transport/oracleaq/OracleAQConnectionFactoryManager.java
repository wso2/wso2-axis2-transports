...
/**
 * Class managing a set of {@link OracleAQConnectionFactory} objects.
 */
public class OracleAQConnectionFactoryManager {

    private static final Log log = LogFactory.getLog(OracleAQConnectionFactoryManager.class);

    /** A Map containing the JMS connection factories managed by this, keyed by name */
    private final Map<String,OracleAQConnectionFactory> connectionFactories =
        new HashMap<String,OracleAQConnectionFactory>();

    /**
     * Construct a Connection factory manager for the JMS transport sender or receiver
     * @param trpInDesc
     */
    public OracleAQConnectionFactoryManager(ParameterInclude trpInDesc) {
        loadConnectionFactoryDefinitions(trpInDesc);
    }

    /**
     * Create OracleAQConnectionFactory instances for the definitions in the transport configuration,
    ...
    private void loadConnectionFactoryDefinitions(ParameterInclude trpDesc) {
        ...
        for (Parameter p : trpDesc.getParameters()) {
            try {
                OracleAQConnectionFactory jmsConFactory = new OracleAQConnectionFactory(p);
     ...
     */
    public OracleAQConnectionFactory getJMSConnectionFactory(String name) {
        return connectionFactories.get(name);
    }

    /**
     * Get the JMS connection factory that matches the given properties, i.e. referring to
     * the same underlying connection factory. Used by the OracleAQSender to determine if already
     ...
     */
    public OracleAQConnectionFactory getJMSConnectionFactory(Map<String,String> props) {
        for (OracleAQConnectionFactory cf : connectionFactories.values()) {
            Map<String,String> cfProperties = cf.getParameters();

            if (equals(props.get(OracleAQConstants.PARAM_CONFAC_JNDI_NAME),
                cfProperties.get(OracleAQConstants.PARAM_CONFAC_JNDI_NAME))
                &&
     ...
     */
    public void stop() {
        for (OracleAQConnectionFactory conFac : connectionFactories.values()) {
            conFac.stop();
        }
    }
